...
def get_events_near(lat: float, lon: float, radius_miles: int = 1) -> list:
    ...
    try:
        ...
        print(f"  Error fetching events at ({lat}, {lon}): {e}")
        return []


def pull_ticketmaster_events(
    model_table_path: str = "model_table_final.csv",
    radius_miles: int = 1
) -> pd.DataFrame:
    df = pd.read_csv(model_table_path)
    if "latitude" not in df.columns or "longitude" not in df.columns:
        raise ValueError("model_table_final.csv must have latitude and longitude columns")
    ...
    for i, row in nta_points.iterrows():
        ...
        if events:
            ...
            all_rows.append({
                "ntaname": row["ntaname"], "event_name": "", "event_type": "",
                "event_genre": "", "event_datetime": "", "venue_name": "", "event_url": ""
            })
        if i % 50 == 0:
            print(f"  {i}/{len(nta_points)} NTAs processed")
        time.sleep(0.2)

    df_events = pd.DataFrame(all_rows)
    print(f"Pulled {len(df_events[df_events['event_name'] != ''])} events across {len(nta_points)} NTAs")
    return df_events


def _dedup_titles(series) -> str:
    seen = set()
    unique = []
    for title in series.dropna():
...
def build_event_features(df_events: pd.DataFrame) -> pd.DataFrame:
    ...
    if has_event.empty:
        return df_events[["ntaname"]].drop_duplicates().assign(
    ...
    agg = (
        ...
        .groupby("ntaname")
        .agg(
            nearby_events_count=("event_name", "nunique"),
            nearby_event_titles=("event_name", _dedup_titles),
            nearby_event_types=("event_type", lambda x: ", ".join(x.dropna().unique()[:3]))
        )
        .reset_index()
    )

    major_types = ["Sports", "Music", "Arts & Theatre"]
    has_event = has_event.copy()
    has_event["is_major"] = has_event["event_type"].isin(major_types)
    major_agg = (
        has_event.groupby("ntaname")["is_major"]
        .max()
        .reset_index()
        .rename(columns={"is_major": "has_major_event"})
    )
    ...
    agg = all_ntas.merge(agg, on="ntaname", how="left")
...
def add_events_to_model_table(
    model_table: pd.DataFrame,
    event_features: pd.DataFrame
) -> pd.DataFrame:
    event_cols = ["nearby_events_count", "nearby_event_titles", "nearby_event_types", "has_major_event"]
    model_table = model_table.drop(columns=[c for c in event_cols if c in model_table.columns])
    model_table = model_table.merge(event_features, on="ntaname", how="left")
...
if __name__ == "__main__":
    ...
    event_features.to_csv("data_event_features.csv", index=False)
    print(f"Saved event features to data_event_features.csv")
